 ...
 */
int AppsModel::query_runapp(int app_id) {
    DBusMessage* msg;
    DBusMessageIter args;
    DBusConnection* conn;
    DBusError err;
...
void AppsModel::query_lockscreen() {
    ...
    conn = dbus_bus_get(DBUS_BUS_SESSION, &err);
    if (dbus_error_is_set(&err)) {
        fprintf(stderr, "%sdbus: connection error: %s.\n",DEBUG_PREFIX, err.message);
        dbus_error_free(&err);
    }

    if (!conn) {
        printf("%sdbus: null connection.\n",DEBUG_PREFIX);
        exit(1);
    }
    ...
    if (dbus_error_is_set(&err)) {
        fprintf(stderr, "%sdbus: name error: %s.\n",DEBUG_PREFIX, err.message);
        dbus_error_free(&err);
    }

    if (DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER != ret &&
        DBUS_REQUEST_NAME_REPLY_ALREADY_OWNER != ret) {
        printf("%sdbus: name owner error.\n",DEBUG_PREFIX);
        exit(1);
    }
    ...
    if (!msg) {
        fprintf(stderr, "%sdbus: null message.\n",DEBUG_PREFIX);
        exit(1);
    }

    // send message
    if (!dbus_connection_send(conn, msg, NULL)) {
        fprintf(stderr, "%sdbus: out of memory.\n",DEBUG_PREFIX);
        exit(1);
    }
